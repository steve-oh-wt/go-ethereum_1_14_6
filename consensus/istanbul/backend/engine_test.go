...
func newBlockchainFromConfig(genesis *core.Genesis, nodeKeys []*ecdsa.PrivateKey, cfg *istanbul.Config) (*core.BlockChain, *Backend) {
	...
	//func NewBlockChain(engine consensus.Engine, vmConfig vm.Config, shouldPreserve func(header *types.Header) bool, txLookupLimit *uint64) (*BlockChain, error) {
	...
	getBlockFn := func() *types.Block {
		return blockchain.GetBlockByHash(blockchain.CurrentBlock().Hash())
	}
	backend.Start(blockchain, getBlockFn, nil)
...
func makeBlockWithoutSeal(chain *core.BlockChain, engine *Backend, parent *types.Block) *types.Block {
	...
	block, _ := engine.FinalizeAndAssemble(chain, header, state, &types.Body{}, nil)
	return block
}
